...
__export(index_exports, {
  ...
  syncAllLanguages: () => syncAllLanguages
});
module.exports = __toCommonJS(index_exports);

// src/scripts/syncData.js
...
function getDirname(importMetaUrl) {
  try {
    return (0, import_path.dirname)((0, import_url2.fileURLToPath)(importMetaUrl));
  } catch {
...
// src/scripts/syncData.js
var import_meta = {};
var __dirname2 = getDirname(import_meta.url);
var langs = ["cn", "jp", "en", "tw", "kr", "th"];
var files = [
  "students.min.json",
  "localization.json",
  "equipment.json",
...
async function downloadJSON(lang, file) {
  const url2 = `https://schaledb.com/data/${lang}/${file}`;
  const outputPath = (0, import_path2.resolve)(__dirname2, `../../assets/data/${lang}/${file}`);
  try {
    ...
    await (0, import_promises.writeFile)(outputPath, json, "utf-8");
...
// src/items/loader.js
var import_meta2 = {};
var __dirname3 = getDirname(import_meta2.url);
var langs2 = ["cn", "jp", "en", "tw", "kr", "th", "vi"];
var itemMap = /* @__PURE__ */ new Map();
var _initted = false;
function initItemMap() {
  if (_initted) return;
  for (const lang of langs2) {
    const filePath = (0, import_path3.resolve)(__dirname3, `../../assets/data/${lang}/items.json`);
    if (!(0, import_fs2.existsSync)(filePath)) continue;
    const data = asArray(JSON.parse((0, import_fs2.readFileSync)(filePath, "utf-8")));
    itemMap.set(lang, data);
  }
  _initted = true;
}
function getItems(lang = "en") {
  initItemMap();
  return itemMap.get(lang) || [];
}
function getItemId(id, lang = "en") {
  initItemMap();
...
function getItemByName(name = "", lang = "en") {
...
function getItemById(id = Number(), lang = "en") {
  return getItemId(Number(id), lang);
}

// src/items/fetch.js
var findItemByName = async (name, lang) => {
  const result = getItemByName(name, lang);
  return result[0] || null;
};
var findItemById = async (id = Number(), lang = "en") => {
  return getItemById(id, lang);
};
var findAllItems = async (lang = "en") => {
  return getAllItems(lang);
};

// src/students/loader.js
var import_fs3 = require("fs");
var import_path4 = require("path");
var import_meta3 = {};
var __dirname4 = getDirname(import_meta3.url);
var langs3 = ["cn", "jp", "en", "tw", "kr", "th", "vi"];
var studentMap = /* @__PURE__ */ new Map();
var _initted2 = false;
var skillKeyMap = {
  Ex: "ex",
  Public: "normal",
...
function initStudentMap() {
  ...
  for (const lang of langs3) {
    const filePath = (0, import_path4.resolve)(__dirname4, `../../assets/data/${lang}/students.min.json`);
    if (!(0, import_fs3.existsSync)(filePath)) continue;
    const data = asArray(JSON.parse((0, import_fs3.readFileSync)(filePath, "utf-8")));
    for (const s of data) {
      const skills = [];
...
function getStudents(lang = "en") {
  initStudentMap();
  return studentMap.get(lang) || [];
}
function getStudentId(id, lang = "en") {
  initStudentMap();
  return studentMap.get(lang)?.find((s) => s.Id === id);
}

// src/students/search.js
function normalize2(name) {
  return name.toLowerCase().replace(/（/g, "(").replace(/）/g, ")");
}
function getStudentByName(name = "", lang = "en") {
  const students = getStudents(lang);
  const found = [];
  const lc = normalize2(name);
  for (const s of students) {
    if ([s?.Name, s?.PathName, s?.DevName].some((n) => n?.toLowerCase() === lc)) {
      return [s];
    }
    if ([s?.Name, s?.PathName, s?.DevName, s?.FamilyName, s?.PersonalName, s?.CharacterVoice].some((n) => n?.toLowerCase().includes(lc))) {
      found.push(s);
    }
  }
  return found;
}
function getAllStudents(lang = "en") {
  return getStudents(lang);
}
function getStudentById(id, lang = "en") {
  return getStudentId(Number(id), lang);
}

// src/students/fetch.js
var findStudentByName = async (name = "", lang = "") => {
  const result = getStudentByName(name, lang);
  return result[0] || null;
};
var findAllStudent = async (lang = "") => {
  const result = await getAllStudents(lang);
  return result;
};
var findStudentById = async (id = Number(), lang = "") => {
